...
func killer_enemy(enemy_type) -> void:
	...
	if enemy_type == 3: # boss raid gold
		...
		Data.data_management["raids"]["raid_gold"]["multiplier"] += 1.0
	...
	if enemy_type == 4: # boss raid citical
		Player.skill_points += 1
		Data.data_management["raids"]["raid_critical"]["multiplier"] += 0.50
...
func prestige_points() -> int:
	...
	var equipment_multiplier: float = (
		...
		+ Player.equipped_items["helm"]["bonus_attributes"]["prestige_points"] \
		+ Player.equipped_items["boot"]["bonus_attributes"]["prestige_points"]
	)
