...
func update_label() -> void:
	...
	box_damage_cost.text = str(
		round(calculate_upgrade_cost(9, 1.20, data["damage"]["level"]))
		)
	...
	box_gold_cost.text = str(
		round(calculate_upgrade_cost(7, 1.15, data["gold"]["level"]))
		)
	...
	box_critical_damage_cost.text = str(
		round(calculate_upgrade_cost(10, 1.15, data["critical_damage"]["level"]))
		)
	...
	box_raid_time_cost.text = str(
		round(calculate_upgrade_cost(15, 1.25, data["raid_time"]["level"]))
		)
	...
	box_prestige_points_cost.text = str(
		round(calculate_upgrade_cost(15, 1.20, data["prestige_points"]["level"]))
		)
	...
	box_skill_duration_cost.text = str(
		round(calculate_upgrade_cost(20, 1.30, Player.skill_duration_level))
		)
	...
	box_skill_cooldown_cost.text = str(
		round(calculate_upgrade_cost(20, 1.30, Player.skill_cooldown_level))
		)
	...
	box_critical_chance_cost.text = str(
		round(calculate_upgrade_cost(75, 1.25, data["critical_chance"]["level"]))
		)
