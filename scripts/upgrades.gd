...
func update_label() -> void:
	...
	box_critical_damage_cost.text = str(
		round(calculate_upgrade_cost(15, 1.15, data["critical_damage"]["level"]))
		)
	...
	box_prestige_points_cost.text = str(
		round(calculate_upgrade_cost(25, 1.05, data["prestige_points"]["level"]))
		)
	...
	box_skill_duration_cost.text = str(
		round(calculate_upgrade_cost(25, 1.15, Player.skill_duration_level))
		)
	...
	box_skill_cooldown_cost.text = str(
		round(calculate_upgrade_cost(25, 1.15, Player.skill_cooldown_level))
		)
